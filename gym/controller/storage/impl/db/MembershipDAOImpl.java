...
public class MembershipDAOImpl implements MembershipsDAO {
    ...
    public Membership update(Membership membership) throws GymException {
        String sql = "UPDATE gym.memberships SET memberships.title=?, memberships.description=?, memberships.number=?, memberships.cost=?, memberships.duration=? WHERE id=?";
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            if (membership.getId() != null) {
                preparedStatement.setLong(6, membership.getId());
            } else {
                throw new GymException("Membership id is null");
            }
            if (preparedStatement.executeUpdate() > 0) {
                return membership;
            }
    ...
    public List<Membership> findByTitle(String title) throws GymException {
        String sql = "SELECT * FROM gym.memberships WHERE title=?";
        try (
                PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            while (resultSet.next()) {
                long membershipId = resultSet.getLong("id");
                String membershipTitle = resultSet.getString("title");
            ...
            throw new GymException("Failed to find membership: " + e.getMessage());
        }
    }
